#!/usr/bin/env python3
"""
ColBase Open Data Expansion Script - 2026-09-23

Target: Add ~80-120 net-new quality tea ceremony ceramics from ColBase
...
MAX_LONG_EDGE = 1400
JPEG_QUALITY = 80
MIN_IMAGE_SIZE = 8000  # 8KB minimum for quality
TARGET_COUNT = 100     # Target ~80-120 new items
BATCH_ID = f"colbase-expansion-{int(time.time() * 1000)}"
...
# Tea keywords for filtering
TEA_KEYWORDS = [
    '茶碗', '茶入', '茶壺', '茶器', '茶杓', '茶盌', '水指', '建水', '蓋置',
    '天目', '楽焼', '楽茶碗', '萩焼', '萩茶碗', '織部', '黒楽', '赤楽',
    '備前', '信楽', '唐津', '青磁茶碗', '瀬戸茶碗', '美濃茶碗',
    '志野', '黄瀬戸', '御本', '高麗茶碗', '井戸茶碗', '刷毛目',
...
def load_existing_accessions():
    ...
    with open(ARTWORKS_PATH, 'r', encoding='utf-8') as f:
        data = json.load(f)
    return {a.get('accessionNumber', '').lower() for a in data if a.get('accessionNumber')}
...
def load_existing_source_urls():
    ...
    with open(ARTWORKS_PATH, 'r', encoding='utf-8') as f:
        data = json.load(f)
    return {a.get('sourceUrl', '').lower() for a in data if a.get('sourceUrl')}
...
def matches_tea_keywords(row):
    """Check if row matches tea-related keywords"""
    title_ja = row.get('title_ja', '')
    genre = row.get('genre', '')
    description = row.get('description', '')
    search_text = f"{title_ja} {genre} {description}".lower()
    
    return any(kw.lower() in search_text for kw in TEA_KEYWORDS)
...
def parse_tsv():
    ...
    with open(TSV_PATH, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            content_status = row[27] if len(row) > 27 else ''
            content_rights = row[28] if len(row) > 28 else ''
            image_url = row[18] if len(row) > 18 else ''
            
            # Filter: internet + open rights + has image
            ...
            candidate = {
                ...
                'description': row[17],
                'image_url': image_url,
                'museum_id': row[19] if len(row) > 19 else '',
                'rights': content_rights,
                'title_en': row[32] if len(row) > 32 else '',
                'period_en': row[37] if len(row) > 37 else '',
                'material_en': row[38] if len(row) > 38 else '',
                'size_en': row[39] if len(row) > 39 else '',
                'holder_en': row[42] if len(row) > 42 else '',
                'description_en': row[43] if len(row) > 43 else '',
            }
            
            # Check tea keywords
            if matches_tea_keywords(candidate):
...
def main():
    print("=" * 60)
    print("ColBase Open Data Teaware Expansion - 2026-09-23")
    ...
    if len(candidates) < 20:
        ...
        if len(candidates) == 0:
            print("No new items to add. Exiting without PR.")
            return 0
    
    # Group by museum for balanced selection
    ...
    # Shuffle for variety across museums
    import random
    ...
    for c in candidates:
        ...
        try:
            ...
            print(f"\n[{len(new_artworks) + 1}/{TARGET_COUNT}] {c['title_ja'] or c['title_en']}")
    ...
    if len(new_artworks) < 20:
        ...
        print("Not creating PR with insufficient items.")
    ...
    log_entry = {
        ...
        "source": "colbase",
        "query": "tea ceramics (茶碗/茶入/天目/楽/萩/織部/備前/信楽/唐津) - Round 2",
        ...
        "note": "ColBase opendata.tsv expansion - Japanese national museum teaware (2026-09-23 batch)",
    }
